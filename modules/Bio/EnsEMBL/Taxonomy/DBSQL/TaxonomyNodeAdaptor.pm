...
package Bio::EnsEMBL::Taxonomy::DBSQL::TaxonomyNodeAdaptor;
...
sub mapper {
  ...
  if (!defined $self->{mapper}) {
	$self->{mapper} = sub {
	  ...
	  if (!$value) {
		$value =
		  Bio::EnsEMBL::Taxonomy::TaxonomyNode->new(-TAXON_ID    => $row[0],
										  -PARENT_ID   => $row[1],
										  -RANK        => $row[2],
										  -ROOT_ID     => $row[3],
										  -LEFT_INDEX  => $row[4],
...
sub _get_node {
  my ($self, $node) = @_;
  if (ref($node) eq 'Bio::EnsEMBL::Taxonomy::TaxonomyNode') {
	return $node;
  }
  else {
